import {
  Component, Input, signal, inject, AfterViewInit,
  ...
  template: `
    ...
    <p-dialog [(visible)]="modalAbierto"
      ...
      <form (ngSubmit)="enviar()" [formGroup]="form" class="space-y-4 pt-2">
        ...
        <div>
          <label class="block text-sm font-medium mb-1" for="cuf-nombre">Nombre *</label>
          <input pInputText id="cuf-nombre" formControlName="nombre" class="w-full"
                 autocomplete="name" />
        ...
        <div>
          <label class="block text-sm font-medium mb-1" for="cuf-email">Email *</label>
          <input pInputText id="cuf-email" type="email" formControlName="email" class="w-full"
                 autocomplete="email" />
        ...
        <div>
          <label class="block text-sm font-medium mb-1" for="cuf-tel">Teléfono (opcional)</label>
          <input pInputText id="cuf-tel" formControlName="telefono" class="w-full"
                 autocomplete="tel" />
        </div>

        <div>
          <label class="block text-sm font-medium mb-1" for="cuf-msg">Mensaje</label>
          <textarea pInputTextarea id="cuf-msg" formControlName="mensaje" rows="3" class="w-full"
...
export class ContactarUnidadFabComponent implements AfterViewInit {
  @Input({ required: true }) unidadId!: string;
  ...
  async enviar() {
    ...
    try {
      await firstValueFrom(
        this.http.post('/api/leads/from-unidad', {
          ...this.form.value,
